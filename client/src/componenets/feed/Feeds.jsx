...
function Feeds({ cuser }) {
    ...
    useEffect(() => {
        ...
        const fetchPosts = async () => {
            try {
                ...
                const res = cuser
                    ? await axios.get("/api/posts/" + cuser._id)
    ...
    }, [cuser, user, postRender]);

    return (
        <div className="flex-[6] lg:flex-[5] min-h-[calc(100vh-64px)] pb-20">
            <div className="mt-4 p-2 md:p-4 max-w-3xl mx-auto">

                {cuser
                    ? cuser._id === user?._id && <Post reRender={setPostRender} />
                    : <Post reRender={setPostRender} />
                }
